...
var laohuji = function(){
    ...
    function init($gameContainer, option){
        ...
        default_.prizes.unshift($.extend(default_.prizes[5], {
            id: '',award_name: ''
        }));
        var template = null;
        if(default_.template){
            ...
            default_.totalH = 104*len;
            default_.prizeH = 104;

            for(var i=0;i<len;i++){
                var prize = default_.prizes[i];
                prizesTpl +=
                    '<div class="g_prize prize1" style="height: '+(100/len)+'%;padding-top: 4%;" prize_num="'+prize.id+'" prize_name="'+prize.award_name+'">' +
                        '<div class="g_prize_bg" style="top: 4%;height: 96%;"></div>' +
                        '<img class="prizeImg" src="'+prize.img+'"/>' +
                    '</div>';
            }
            template = [];
            ...
            template.push(              '<div class="message_b"><div>中奖说明：摇中三张相同图片，即获得奖品。活动规则由网贷天眼在法律允许范围内依法解释。</div></div>');
            template.push(          '</div>');
            template.push(          '<div class="g_prize_container">');
            template.push(              '<div class="g_prize_container1">');
            template.push(                  '<div class="g_prizes" style="height:'+default_.totalH+'%;">');
	...
	domCache.laohuji_results_server = null;
        ...
        $('#g_machine').on("touchstart", function(e){
            ...
            if ($t.hasClass('lucky_draw')) {//老虎机抽奖
                ...
                var prizesH = default_.totalH - default_.prizeH*2;
        ...
        _num.css('bottom', 0).animate({
            ...
            complete: function(_num, _i){
                ...
                if(results){
                    ...
                    _num.css('bottom', 0).animate({
                        bottom: -(prizeH*(container.children().length-2)*4)
                                -((prizeH)*(container.children().length-1 -prizeIndex))+"%"
                    },{
                        duration: 3000,
                        easing: "easeOutCirc",

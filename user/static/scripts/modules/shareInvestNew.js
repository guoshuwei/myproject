...
$('body')
    ...
    .on('click','[role=shareInvest]',function(){
        ...
        shareInvest.show(function(){
            var pieColor = ['#5793f2','#dc567e','#bc3b45','#dd4542','#fc9c36','#fbc633','#d2df59','#66d16b','#42b9a5','#3d91be','#6076e0','#a428be'];

            // 平台分布
            var echartDistributionShare = echarts.init(document.getElementById("echart-distribution-share"));

            var echartDistributionShareOption = {
                color : pieColor,
                tooltip : {
                    trigger: 'item',
            ...
            echartDistributionShare.setOption(echartDistributionShareOption);
            setTimeout(function(){
                if(closed) return;
                _echart_distribution_status = echartDistributionShare.getDataURL({type:"png",backgroundColor:"#fff"});
            ...
            // 流动性
            var echartMobilityShare = echarts.init(document.getElementById("echart-mobility-share"));

            var echartMobilityShareOption = {
                color : pieColor,
                tooltip : {
                    trigger: 'item',
                    formatter: "{a} <br/>{b} : {c} ({d}%)"
            ...
            echartMobilityShare.setOption(echartMobilityShareOption);
            setTimeout(function(){
                if(closed) return;
                _echart_mobility_status = echartMobilityShare.getDataURL({type:"png",backgroundColor:"#fff"});
                _echart_load_num = _echart_load_num + 1;
                if(_echart_load_num == 2){
                    doShare();
                }

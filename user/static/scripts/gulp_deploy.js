...
module.exports = function(){
	...
	for(var i in arguments){
		_process[i] = arguments[i];
	}
	if(_process.length<4){
		deploy.error();
		return;
	}

	//处理脚本传参 
	var args = _process.slice(2),
		len = args.length,
		i =0,
	...
	*/
	...
	var deployPath = collapse(dir(_process.slice(1,2)[0]));
	var modPath = resolve(dir(_process.slice(3,4)[0]),deployPath);
	for(;i<len;i++){

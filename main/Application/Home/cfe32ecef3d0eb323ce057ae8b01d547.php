    ...
    <div class="index_body">
        <div class="index_page">
            <div class="page_cont">
                <div class="page_nav">
                    <div class="nav_title">
                        <p class="title_p1">专家介绍</p>
                        <p class="title_p2">Departments</p>
                    </div>
                    <ul class="hd nav_ul">
                        <?php if(is_array($perintro)): foreach($perintro as $k=>$perintro): if((isset($_GET['sid']) AND ($perintro['id'] == $_GET['sid']))): ?><li class="sublist active"><?php echo ($perintro["title"]); ?></li>
                            <?php elseif((!isset($_GET['sid']) AND ($k == 0))): ?>
                                <li class="sublist active"><?php echo ($perintro["title"]); ?></li>
                            <?php else: ?>
                                <li class="sublist"><?php echo ($perintro["title"]); ?></li><?php endif; endforeach; endif; ?>
                    </ul>
                </div>
                <div class="bd page_introduce smt">
                    <?php if(is_array($re_perintro)): foreach($re_perintro as $k=>$re_perintro): if((isset($_GET['sid']) AND ($re_perintro['id'] == $_GET['sid']))): ?><ul class="item active">
                                <li>
                                	<?php echo stripslashes($re_perintro['content']); ?>
                                </li>
                            </ul>
                        <?php elseif((!isset($_GET['sid']) AND ($k == 0))): ?>
